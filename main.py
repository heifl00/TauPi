...
def welcome_message():
    ...
    welcome = ("<pre>Hallo! Hier ist der Bot-TauPi! Hier findest </pre>"
               "<pre>du die Uebersicht über alle Befehle zur"
               "<pre>Steuerung von TauPi: </pre> \n"
               "<pre>----------------------------\n"
               "   --Einmalige Abfragen-- \n"
               "----------------------------</pre>\n"
               "<pre> - zeigt beim benutzen eines Befehls </pre>"
               "<pre>einmalig die gewuenschten Werte des </pre>" 
               "<pre>Schimmelwaechters an.</pre> \n"
               "/bme <pre> = zeigt alle Werte des BME280 an</pre>\n"
               "/temp <pre> = zeigt die </pre> \n"
               "<pre>Oberflaechentemperatur an</pre>\n"
               "/humi <pre> = zeigt die rel. </pre>"
               "<pre>Luftfeuchtigkeit an</pre>\n"
               "<pre>----------------------------\n"
               " --Automatische Abfragen--\n"
               "----------------------------</pre>\n"
               "<pre> - zeigt bei eingeschalteter Funktion </pre>"
               "<pre>kontinuierlich alle Werte des </pre>"
               "<pre>Schimmelwaechters an</pre> \n"
               "/auto_on <pre> = schaltet die automatischen </pre>"
               "<pre>Ausgabe an</pre> \n"
               "/auto_off <pre> = schaltet die automatischen </pre> \n"
               "<pre>Ausgabe ab</pre> \n")
...
def SchiPi_Warnung():
    ...
    if result[4] == True:
        text = ('Es besteht keine Schimmelgefahr die Ergebnisse' +
                'sehen wie folgt aus: ' + '\n')
    else:
        text = ('Es besteht Schimmelgefahr die Ergebnisse sehen' +
                'wie folgt aus:' + '\n')
...
def get_message(message):
    text = ('<pre>TauPi kann mit dieser Nachricht nichts </pre>'
            '<pre>anfangen. Beim klicken auf Menu findest </pre>'
            '<pre>du alle vorhandenen Befehle.</pre>')
    bot.reply_to(message['message']['chat']['id'], text, 
...
def bme280_temp(message):
    temp = bme280_data.bme280_temp()
    text = ("<pre>Oberflächentemperatur:   {:<5.2f} &#176;C</pre>"
           ).format(temp)
    bot.send(message['message']['chat']['id'], text)
...
def bme280_humi(message):
    humi = bme280_data.bme280_humi()
    text = ("<pre>rel. Luftfeuchtigkeit:   {:<5.2f} %</pre>"
           ).format(humi)
    bot.send(message['message']['chat']['id'], text)
...
def auto_values_on(message):
    bot.send(message['message']['chat']['id'], 
             '<pre>Es werden ab jetzt immer automatisch ' + 
             'die aktuellen Daten angezeigt.' + '\n' +
             'Möchtest du das beenden ' + 
             'benutze den Befehl </pre>/auto_off.')
...
def auto_values_off(message):
    bot.send(message['message']['chat']['id'], 
             '<pre>Es werden ab jetzt keine aktuellen ' + 
             'Daten mehr automatisch angezeigt.' + '\n' +
             'Möchtest du automatisch dir die ' + 
             'aktuellen Daten anzeigen lassen, ' + 
             'benutze den Befehl </pre>/auto_on.')
    global print_auto_values
    print_auto_values = False
...
# Register the 'reply_ping' function to be called 
# when '/ping' command is received
bot.register('/ping', reply_ping)
# Register the 'overview' function to be called
# when '/overview' command is received
bot.register('/overview', overview)

# Register the 'bme280_all_values' function to be called
# when '/bme' command is received
bot.register('/bme', bme280_all_values)
# Register the 'bme280_temp' function to be called
# when '/temp' command is received
bot.register('/temp', bme280_temp)
# Register the 'bme280_humi' function to be called
# when '/humi' command is received
bot.register('/humi', bme280_humi)

# Register the 'auto_values_on' function to be called
# when '/auto_on' command is received
bot.register('/auto_on', auto_values_on)
# Register the 'auto_values_off' function to be called
# when '/auto_off' command is received
